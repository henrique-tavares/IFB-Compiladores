...
int ParamTable[PARAMTABLE_SIZE]; /* Lista de parâmetros formais para os
                                    procedimentos */
...
/* Tratamento da tabela de sí­mbolos */
...
void Init() {
  ...
  for (i = 0; i < SYMBOLTABLE_SIZE; i++) SymbolTable[i] = ' ';
...
void Undefined(char name) { Abort("Undefined identifier %c", name); }

/* Avisa a respeito de um identificador desconhecido */
void Duplicate(char name) { Abort("Duplicated identifier %c", name); }

/* Avisa a respeito de uma palavra-chave desconhecida */
void Unrecognized(char name) { Abort("Unrecognized keyword %c", name); }

/* Avisa a respeito de um identificador que não é variável */
void NotVar(char name) { Abort("'%c' is not a variable", name); }
...
/* Verifica se "name" consta na tabela de sí­mbolos */
char InTable(char name) { return (SymbolTable[name - 'A'] != ' '); }

/* Adiciona nova entrada à  tabela de sõ­mbolos */
void AddEntry(char name, char type) {
  if (InTable(name)) Duplicate(name);
...
void CheckVar(char name) {
  ...
  if (SymbolType(name) != 'v') NotVar(name);
}

/* Limpa a tabela de parâmetros formais */
void ClearParams() {
  int i;
  for (i = 0; i < PARAMTABLE_SIZE; i++) ParamTable[i] = 0;
  ParamCount = 0;
}

/* Retorna número indicando a posição do parâmetro */
int ParamNumber(char name) { return ParamTable[name - 'A']; }

/* Verifica se nome é parâmetro */
int IsParam(char name) { return (ParamTable[name - 'A'] != 0); }
...
/* Emite código para o epõ­logo de um programa */
void AsmEpilog() {
  EmitLn("MOV AX, 4C00h");
  EmitLn("INT 21h");
}
...
void AsmStoreVar(char name) { EmitLn("MOV [%c], AX", name); }
...
void AsmCall(char name) { EmitLn("CALL %c", name); }

/* Retorno de sub-rotina */
void AsmReturn() { EmitLn("RET"); }

/* Calcula deslocamento do parâmetro na pilha */
int AsmOffsetParam(int par) {
  int offset;

  /* Offset = (endereço de retorno + BP) + tamanho do parâmetro * posição
   * relativa */
...
void AsmCleanStack(int bytes) {
  if (bytes > 0) EmitLn("ADD SP, %d", bytes);
}
...
/* Escreve o epõ­logo para um procedimento */
void AsmProcEpilog() {
  EmitLn("POP BP");
  EmitLn("RET");
}
