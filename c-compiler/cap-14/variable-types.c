...
void DumpTable() {
  ...
  for (i = 0; i < SYMBOLTABLE_SIZE; i++)
    if (SymbolTable[i] != '?') printf("%c = %c\n", i + 'A', SymbolTable[i]);
}

/* Retorna o tipo de um identificador */
char SymbolType(char name) { return SymbolTable[name - 'A']; }
...
void AsmPush(char type) {
  if (type == 'b') EmitLn("CBW"); /* Só é possível empilhar "word"s */
  if (type == 'l') EmitLn("PUSH DX");
  EmitLn("PUSH AX");
}
...
/* Faz a promoção dos tipos dos operandos e inverte a ordem dos mesmos se
 * necessário */
char AsmSameType(char t1, char t2, int ordMatters) {
  int swaped = 0;
  int type = t1;
  ...
  if (!swaped && ordMatters) AsmSwap(type);
...
int IsVarType(char c) { return (c == 'b' || c == 'w' || c == 'l'); }
