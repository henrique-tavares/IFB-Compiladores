...
char *KeywordList[KEYWORDLIST_SIZE] = {
    "IF", "ELSE", "ENDIF", "WHILE", "ENDWHILE", "READ", "WRITE", "VAR", "END"};
...
void Undefined(char *name) { Abort("Undefined identifier %s\n", name); }

/* Avisa a respeito de um identificador duplicado */
void Duplicate(char *name) { Abort("Duplicated identifier %s\n", name); }
...
void GetName() {
  ...
  SkipWhite();
  if (!isalpha(Look)) Expected("Identifier or Keyword");
...
/* Se a string de entrada estiver na tabela, devolve a posição ou -1 se não
 * estiver */
...
void Scan() {
  ...
  if (Token == 'x') {
    ...
    if (kw >= 0) Token = KeywordCode[kw];
  }
}

/* Verifica se a string combina com o esperado */
void MatchString(char *s) {
  if (strcmp(TokenText, s) != 0) Expected(s);
  NextToken();
}
...
int Locate(char *name) { return Lookup(name, SymbolTable, SymbolCount); }
...
void AddEntry(char *name, char type) {
  ...
  if (newSymbol == NULL) Abort("Out of memory!");
